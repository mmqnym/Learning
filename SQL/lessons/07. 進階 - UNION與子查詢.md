...
關聯子查詢代表著會參考其外部查詢的值來做一次查詢，效能上會是一個大負擔，因為外部查詢每處理一行資料，子查詢就會執行一次，導致在資料量大時執行速度變慢，效率低下。因此，建議盡量將關聯子查詢改寫為使用 JOIN 操作，或是利用資料庫的「去關聯化」優化。
...
WHERE (
	SELECT
		AVG(sales) FROM employees AS e2
	WHERE 
		e2.city = e1.city
)
```
...
由於這種子查詢的效能非常的差，不建議過多的學習關聯子查詢，通常在這種情況下，會將子查詢改寫為 JOIN 操作。

```sql
SELECT 
	e1.first_name,
	e1.sales,
	city_stats.min_sales
FROM 
	employees e1
INNER JOIN (
	SELECT 
...
ON 
	e1.city = city_stats.city
WHERE 
	e1.sales = city_stats.avg_sales;
```
...
ROLLBACK 的具體使用方式與情境將於下一個主題說明，本主題聚焦一切正確運行的 Transactions 上。
...
當使用 Begin 開始了一個 Transactions 時，可以用 **ROLLBACK** 來回復到操作前的狀態，用於撤銷想要回復的操作。
...
-- 也可以使用儲存點的機制（安全點僅在當前會話中有效）
-- 退回到 label
-- *當直接使用 ROLLBACK 會直接結束這次的 Transaction
-- *但是如果使用的是 ROLLBACK TO SAVEPOINT 就仍在當前的 Transaction 中
BEGIN;
<statement_1>;
<statement_2>;
SAVEPOINT <label>;
...
在 SQL 中，由於函數內不能使用事務相關的操作，Procedure 可以用來處理這種情況，用於要執行一系列查詢時使用，並且支援回滾操作。
...
-- 不一定要寫，在 Procedure 中，RETURN 僅代表要直接結束這個 Procedure
-- 等同於一般程式語言的 void 函數使用 return 顯式結束
-- Procedure 是不能回傳非空值的
RETURN;
END;
$$
...
CREATE PROCEDURE transfer(sender_id INT, receiver_id INT, amount INT)
LANGUAGE plpgsql
AS
$$
BEGIN
...
-- 因爲這個範例多寫了一個 EXCEPTION 區塊來接住所有例外
-- 先前的餘額不足例外也會在這邊處理，而不是自動 rollback
-- 對於簡單的例外處理可以不需要寫 EXCEPTION 區塊
EXCEPTION
  -- OTHERS 代表所有例外
  WHEN OTHERS THEN
    ROLLBACK;
    -- 紀錄訊息，SQLERRM 可以取得原錯誤訊息
...
-- 這個查詢方式效能並不好
-- 後續學到 CTE 可以大幅提升效能
-- 這題也可以使用 Self JOIN 與 CASE 來解決，效能會好一些
...
DECLARE
	tmp_position_1 TEXT;
	tmp_salary_1 NUMERIC(8, 2);
	tmp_position_2 TEXT;
	tmp_salary_2 NUMERIC(8, 2);
BEGIN
	SELECT 
		position_title, 
		salary
	...
	UPDATE employees
	SET
		position_title = tmp_position_2,
		salary = tmp_salary_2
	WHERE emp_id = emp1;

	UPDATE employees
	SET
		position_title = tmp_position_1,
		salary = tmp_salary_1
	WHERE emp_id = emp2;
